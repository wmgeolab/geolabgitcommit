...
def pyGitUpdate(repository_name):
  ...
  for (dirpath, dirnames, filenames) in os.walk('.'):
      for f in filenames:
          if(os.stat(os.path.join(dirpath, f)).st_size >= 1e+8):
            print("File " + str(os.path.join(dirpath,f)) + " is too large to commit to github (>100 MB).  It will NOT be backed up via git!")
